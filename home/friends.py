...
Names are represented as strings and are case sensitive. Connections are undirected,
so if "sophia" is connected with "nikola", then it's also correct in reverse.
...
Connections can be repeated in the initial data,
...
class Friends(set):
    ...
    def connected(self, name):
        name_set = {name}
        return {min(item - name_set) for item in self if item > name_set}
