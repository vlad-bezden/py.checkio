...
- 'conjunction' denoted x ∧ y, satisfies x ∧ y = 1 if x = y = 1 and x ∧ y = 0
    otherwise.
- 'disjunction' denoted x ∨ y, satisfies x ∨ y = 0 if x = y = 0 and x ∨ y = 1
    otherwise.
...
- 'exclusive' (exclusive or) denoted x ⊕ y and can be described as (x ∨ y)∧ ¬
    (x ∧ y). It excludes the possibility of both x and y. Defined in terms of
    arithmetic it is addition mod 2 where 1 + 1 = 0.
- 'equivalence' denoted x ≡ y and can be described as ¬ (x ⊕ y). It's true
    just when x and y have the same value.
...
operation name as described earlier. You should calculate the value and return
it as 1 or 0.
